...
int hasGreatGrandchild(BTNode *node)
{
    ...
    int leftDetph = 0;
    int rightDepth = 0;
    leftDetph += hasGreatGrandchild(node->left);
    rightDepth += hasGreatGrandchild(node->right);
    if (leftDetph >= 3 || rightDepth >= 3)
    {
        printf("%d ", node->item);
    }
    return leftDetph <= rightDepth ? rightDepth+1 : leftDetph+1;
}
